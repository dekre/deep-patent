...
def extract_kapture_global_features(
    ...
    with get_all_tar_handlers(
        ...
        imgs = [
            image_name for _, _, image_name in kapture.flatten(kdata.records_camera)
        ]
        ...
        if global_features_type not in kdata.global_features:
            kdata.global_features[global_features_type] = kapture.GlobalFeatures(
                "dirtorch", gfeat_dtype, gfeat_dsize, "L2"
            )
            global_features_config_absolute_path = get_feature_csv_fullpath(
                kapture.GlobalFeatures, global_features_type, kapture_root_path
            )
            global_features_to_file(
        ...
        for i in tqdm.tqdm(range(dataset.nimg)):
            ...
            global_feature_fullpath = get_global_features_fullpath(
                global_features_type, kapture_root_path, image_name, tar_handlers
            )
            gfeat_i = bdescs[i, :]
...
if __name__ == "__main__":
    ...
    if args.global_features_type is None:
        args.global_features_type = os.path.splitext(os.path.basename(args.checkpoint))[
            0
        ]
    ...
    # Evaluate
    res = extract_kapture_global_features(
        args.kapture_root,
        net,
