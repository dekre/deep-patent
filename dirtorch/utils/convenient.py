...
def assert_outpath(path, ext="", mkdir=False):
    ...
    if ext:
        assert os.path.splitext(fname)[1] == ext, "Bad file extension, should be " + ext
    if mkdir:
        _mkdir(folder, isfile=False)
...
def toc(tag="", cum=False):
    ...
    if cum:
        ...
        if cum == "avg":
            t /= nb
    print(
        "%selpased time since %s = %gs"
        % ({False: "", True: "cumulated ", "avg": "average "}[cum], tag, t)
    )
    return t
