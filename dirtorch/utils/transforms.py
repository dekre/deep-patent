...
def create(cmd_line, to_tensor=False, **vars):
    ...
    try:
        ...
        raise SyntaxError(
            "Cannot interpret this transform list: %s\nReason: %s" % (cmd_line, e)
        )
...
class Scale(object):
    ...
    def __init__(
        ...
        if isinstance(self.size, float):
            assert (
                0 < self.size <= 4
            ), "bad float self.size, cannot be outside of range ]0,4]"
    ...
    def get_params(self, imsize):
        ...
        if isinstance(self.size, int):
            ...
            if (is_smaller(w, h) and w == self.size) or (
                is_smaller(h, w) and h == self.size
            ):
                ow, oh = w, h
            elif is_smaller(w, h):
                ow = self.size
...
class RandomScale(Scale):
    ...
    def get_params(self, imsize):
        ...
        size = max(
            min(
                int(0.5 + F.rand_log_uniform(self.min_size, self.max_size)),
                self.max_size,
            ),
            self.min_size,
        )
...
class RandomErasing(StillTransform):
    ...
    def _transform(self, image):
        ...
        if not (
            w_occlusion_min < w_occlusion_max and h_occlusion_min < h_occlusion_max
        ):
            return image
        ...
        if len(image.getbands()) == 1:
            ...
            rectangle = Image.fromarray(
                np.uint8(
                    np.random.rand(w_occlusion, h_occlusion, len(image.getbands()))
                    * 255
                )
            )
...
class BBoxToPixelLabel(object):
    ...
    def __call__(self, inp):
        ...
        if self.mode == "hough":
            ...
            topos = lambda left, pos, right: np.floor(
                self.nbin * (pos - left) / (right - left)
            )

            def tolog(size):
                size = max(size, 1e-8)  # make it positive
                return np.round(
                    np.log(size / self.ref_scale) / np.log(self.log_scale)
                    + (self.nbin - 1) / 2
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--trfs", type=str, required=True, help="sequence of transformations"
    )
    ...
    parser.add_argument(
        "--layout", type=int, nargs=2, default=(3, 3), help="Number of rows and columns"
    )
