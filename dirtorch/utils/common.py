    ...
    if cuda:
        if any(gpu >= 1000 for gpu in gpus):
            ...
            os.environ["CUDA_VISIBLE_DEVICES"] = ",".join(
                [str(visible_gpus[gpu - 1000]) for gpu in gpus]
            )
        else:
...
def transform(pca, X, whitenp=0.5, whitenv=None, whitenm=1.0, use_sklearn=True):
    if use_sklearn:
        ...
        if pca.whiten:
            X_transformed /= whitenm * np.power(
                pca.explained_variance_[:whitenv], whitenp
            )
    else:
        X = X - pca["means"]
...
def whiten_features(
    X, pca, l2norm=True, whitenp=0.5, whitenv=None, whitenm=1.0, use_sklearn=True
):
    res = transform(
        pca,
        X,
