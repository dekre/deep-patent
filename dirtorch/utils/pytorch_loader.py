...
class PytorchLoader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.img_and_target_transform:
            ...
            if "bbox" in self.output:
                ...
                if bbox:
                    img_and_label["bbox"] = bbox

            if any(a.endswith("_map") for a in self.output):
                original_polygons = try_to_get(
                    self.dataset.get_polygons, index, toint=True
                )
            ...
            if "label_map" in self.output:
                pixlabel = self.dataset.get_label_map(
                    index, imsize, polygons=transformed_polygons
                )
            ...
            for out_key in self.output:
                for type in ["_instance_map", "_angle_map"]:
                    if not out_key.endswith(type):
                        continue
                    cls = out_key[: -len(type)]
                    get_func = getattr(self.dataset, "get" + type)
                    pixlabel = get_func(
                        index, cls, imsize, polygons=transformed_polygons
                    )
        ...
        for o in self.output:
            assert img_and_label.get(o) is not None, "Missing field %s for img %s" % (
                o,
                img_filename,
            )
    ...
    def __repr__(self):
        ...
        if self.img_and_target_transform:
            ...
            fmt_str += "{0}{1}\n".format(
                tmp,
                repr(self.img_and_target_transform).replace(
                    "\n", "\n" + " " * len(tmp)
                ),
            )
        if self.transform:
...
class BalancedSampler(data.sampler.Sampler):
    ...
    def __init__(self, dataset, size=1.0, balanced=1.0, use_all=False):
        ...
        self.npc = np.percentile(
            [len(imgs) for imgs in self.cls_imgs], max(0, min(50 * size, 100))
        )
