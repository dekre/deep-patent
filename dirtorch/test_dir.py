...
def eval_model(
    ...
    if not load_feats:
        ...
        for trfs in trfs_list:
            ...
            qdescs.append(
                bdescs[-1]
                if db is query_db
                else extract_image_features(query_db, trfs, net, desc="query", **kw)
            )
    ...
    try:
        aps = [
            db.eval_query_AP(q, s) for q, s in enumerate(tqdm.tqdm(scores, desc="AP"))
        ]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dataset", "-d", type=str, required=True, help="Command to load dataset"
    )
    ...
    parser.add_argument(
        ...
        default="",
        nargs="+",
        help="test transforms (can be several)",
    )
    parser.add_argument(
        "--pooling",
        type=str,
        default="gem",
        help="pooling scheme if several trf chains",
    )
    ...
    parser.add_argument(
        "--detailed", action="store_true", help="return detailed evaluation"
    )
    parser.add_argument(
        "--save-feats", type=str, default="", help="path to output features"
    )
    parser.add_argument(
        "--load-feats", type=str, default="", help="path to load features from"
    )

    parser.add_argument(
        "--threads", type=int, default=8, help="number of thread workers"
    )
    ...
    # post-processing
    parser.add_argument(
        "--whiten", type=str, default="Landmarks_clean", help="applies whitening"
    )

    parser.add_argument(
        "--aqe", type=int, nargs="+", help="alpha-query expansion paramenters"
    )
    parser.add_argument(
        "--adba", type=int, nargs="+", help="alpha-database augmentation paramenters"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.25,
        help="whitening power, default is 0.5 (i.e., the sqrt)",
    )
    parser.add_argument(
        "--whitenv",
        type=int,
