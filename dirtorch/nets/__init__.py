...
from .backbones.resnet import resnet18, resnet50, resnet101, resnet152
from .rmac_resnet import resnet18_rmac, resnet50_rmac, resnet101_rmac, resnet152_rmac
from .rmac_resnet_fpn import (
    resnet18_fpn_rmac,
    resnet50_fpn_rmac,
    resnet101_fpn0_rmac,
    resnet101_fpn_rmac,
...
def create_model(arch, pretrained="", delete_fc=False, *args, **kwargs):
    ...
    model.preprocess = dict(
        mean=model.rgb_means, std=model.rgb_stds, input_size=max(model.input_size)
    )
    ...
    if os.path.isfile(pretrained or ""):
        ...
        ), "Model %s must be initialized with a valid model file (not %s)" % (
            arch,
            pretrained,
        )
    ...
    for k, v in list(d.items()):
        if k not in new_dict:
            if not k.endswith("num_batches_tracked"):
                print(
                    "Loading weights for %s: Missing layer %s" % (type(net).__name__, k)
                )
            new_dict[k] = v
        elif v.shape != new_dict[k].shape:
            print(
                "Loading weights for %s: Bad shape for layer %s, skipping"
                % (type(net).__name__, k)
            )
            new_dict[k] = v
