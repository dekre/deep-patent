...
class Dataset(object):
    ...
    c_relevant_idx = (
        {}
    )  # images belonging to each class (c_relevant_idx[cl_name] = [idx list])
    ...
    def get_image(self, img_idx, resize=None):
        ...
        if resize:
            img = img.resize(
                resize,
                Image.ANTIALIAS
                if np.prod(resize) < np.prod(img.size)
                else Image.BICUBIC,
            )
        return img
    ...
    def eval_query_AP(self, query_idx, scores):
        ...
        if sum(gt[keep] > 0) == 0:
            return -1  # exclude queries with no relevants form the evaluation
    ...
    def __repr__(self):
        ...
        if self.nclass:
            res += ", %d classes" % (self.nclass)
        if self.ninstance:
            res += ", %d instances" % (self.ninstance)
        try:
...
class CatDataset(Dataset):
    ...
    def __init__(self, *datasets):
        ...
        for db in datasets:
            assert (
                db.nclass == self.nclass
            ), "All dataset must have the same number of classes"
    ...
    def __getattr__(self, name):
        ...
        assert not callable(
            val
        ), "CatDataset: %s is not a shared attribute, use call()" % str(name)
        for db in self.datasets[1:]:
            assert np.all(
                val == getattr(db, name)
            ), "CatDataset: inconsistent shared attribute %s, use get()" % str(name)
        return val
...
def deploy(dataset, target_dir, transforms=None, redo=False, ext=None, **savekwargs):
    ...
    try:
        ...
        try:
            os.makedirs(os.path.split(imsizes_path)[0])
        except OSError:
            pass
    ...
    if i + 1 < len(dataset):
        updated += sum(
            job_utils.parallel_threads(
                range(i + 1, len(dataset)),
                check_one_image,
                desc="Deploying dataset",
                n_threads=0,
                front_num=0,
            )
        )
...
class DeployedDataset(Dataset):
        ...
        if "/query_db/" not in root:
            try:
                ...
                if query_db is not dataset:
                    self.query_db = deploy(
                        query_db,
                        os.path.join(root, "query_db"),
                        transforms=trfs,
                        ext=ext,
                    )
    ...
    def get_key(self, i):
        ...
        if self.ext:
            key = os.path.splitext(key)[0] + "." + self.ext
        return key
    ...
    def get_label_map(self, i, *args, **kw):
        assert (
            "polygons" in kw
        ), "you need to supply polygons because image has been transformed"
...
def deploy_and_split(
    ...
    **_useless
    ...
    traindb = deploy(
        trainset, deploy_dir, transforms=deploy_trf, ext=img_ext, quality=img_quality
    )

    if valset:
        # load a validation db
        valdb = deploy(
            valset, deploy_dir, transforms=deploy_trf, ext=img_ext, quality=img_quality
        )
...
class CropDataset(Dataset):
    ...
    def get_image(self, img_idx):
        ...
        return crop_signs[
            0
        ]  # temporary use one, but have to change for multiple signages
    ...
    def crop_image(self, img, polygons):
        ...
        for Polycc in polygons:
            ...
            warpimg = Image.fromarray(
                cv2.warpPerspective(rgbimg, M, (645, 800))
            )  # from cv2 type rgbimg
            crop_sign = warpimg.crop(
                [
                    np.min(Poly_d[:, 0]),
                    np.min(Poly_d[:, 1]),
                    np.max(Poly_d[:, 0]),
