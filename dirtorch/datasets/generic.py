...
class ImageListRelevants(Dataset):
    ...
    def __init__(self, gt_file, root=None, img_dir="jpg", ext=".jpg"):
        ...
        with open(gt_file, "rb") as f:
            gt = pickle.load(f)
        self.imgs = [
            osp.splitext(e)[0] + (osp.splitext(e)[1] if osp.splitext(e)[1] else ext)
            for e in gt["imlist"]
        ]
        self.qimgs = [
            osp.splitext(e)[0] + (osp.splitext(e)[1] if osp.splitext(e)[1] else ext)
    ...
    def get_query_filename(self, qimg_idx, root=None):
        return os.path.join(
            root or self.root, self.img_dir, self.get_query_key(qimg_idx)
        )
    ...
    def eval_query_AP(self, query_idx, scores):
        ...
        if self.relevants:
            ...
            assert gt.shape == scores.shape, "scores should have shape %s" % str(
                gt.shape
            )
            ...
            for mode in ("easy", "medium", "hard"):
                ...
                if (
                    sum(gt[keep] > 0) == 0
                ):  # exclude queries with no relevants from the evaluation
                    d[mode] = -1
                else:
